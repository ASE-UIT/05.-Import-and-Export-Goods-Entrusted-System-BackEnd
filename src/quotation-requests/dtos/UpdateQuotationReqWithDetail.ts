...
export const UpdateQuoteReqWithDetailSchema = z
  .object({
    requestDate: z.coerce
      .date()
      .describe('The request date of the quote request')
      .optional(),
    //userId: z.string().describe("The customer ID whose create the quote request").optional(),
    status: z
      .nativeEnum(QuotationReqStatus)
      .describe('The status of the quote request')
      .optional(),
    origin: z
      .string()
      .min(1, 'Origin is required')
      .describe('The origin of the shipment')
      .optional(),
    destination: z
      .string()
      ...
      .optional(),
    shipmentReadyDate: z.coerce
      .date()
      .describe('The ready date for the shipment')
      .optional(),
    shipmentDeadline: z.coerce
      .date()
      .describe('The deadline for the shipment')
      .optional(),
    cargoInsurance: z
      .boolean()
      .describe('Whether the shipment has insurance')
      .optional(),
    shipmentType: z
      .nativeEnum(ShipmentType)
      .describe('The type of shipment')
      .optional(),
    packageType: z
  ...
  .refine(
    ...
    {
      message:
        'Dates must follow the order: requestDate <= shipmentReadyDate <= shipmentDeadline',
      path: ['requestDate', 'shipmentReadyDate', 'shipmentDeadline'], // Bạn có thể chỉ định một trường cụ thể nếu muốn
    },
  );

export class UpdateQuoteReqWithDetailDto extends createZodDto(
